import { gateway, type Tool, streamText } from "ai";
...
export function createShoppingAgent({ userId }: ShoppingAgentOptions) {
  ...
  // Configure the gateway with the API key
  const anthropicModel = gateway("anthropic/claude-sonnet-4.5", {
    apiKey: process.env.AI_GATEWAY_API_KEY,
  });

  // Return the configuration instead of creating an agent instance
  // This allows the caller to use streamText with the proper configuration
  return {
